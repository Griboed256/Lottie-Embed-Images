...
class ReusableThreadingTCPServer(socketserver.ThreadingTCPServer):
    # Отключаем reuse_address на Windows для защиты от дублирования портов
...
class PreviewServerManager:
    ...
    def stop(self, log_fn: Optional[Callable] = None, wait_timeout: float = 1.0) -> None:
        ...
        # Жестко закрываем сокет первыми, чтобы моментально освободить порт!
        ...
        try:
            # Запускаем фоновый процесс отключения и даже не ждем его
    ...
    def start(self, output_jsons: List[Path], log_fn: Optional[Callable] = None) -> None:
        ...
        paths_dict = {}
        cache_buster = int(time.time() * 1000) # <--- Умножили на 1000
        ...
        class BatchHandler(http.server.BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                # Отрезаем GET-параметры (?v=12345), чтобы роутинг работал корректно
                parsed_path = self.path.split('?')[0]
                
                if parsed_path in ('/', '/test.html'):
                    self.send_response(200)
                    self.send_header("Content-type", "text/html; charset=utf-8")
                    self.send_header("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0") # Запрет кэша
                    self.end_headers()
                    self.wfile.write(full_html.encode('utf-8'))
                elif parsed_path == '/lottie.js':
                    self.send_response(200)
                    self.send_header("Content-type", "application/javascript")
                    self.send_header("Cache-Control", "public, max-age=31536000") # JS можно кэшировать
                    self.end_headers()
                    self.wfile.write(lottie_js.encode('utf-8'))
                elif parsed_path in paths_dict:
                    try:
                        with open(paths_dict[parsed_path], 'rb') as f:
                            content = f.read()
                        self.send_response(200)
        ...
        url = f"http://localhost:{chosen_port}/test.html?t={cache_buster}"
...
# ── Водяной знак (правый нижний угол) ─────────────────────────────────
    ...
    def _build_tab_info(self):
        ...
        self._create_accordion(self.tab_info, "📘", "КАК ПОЛЬЗОВАТЬСЯ ПРИЛОЖЕНИЕМ", 
            "1. Выбери исходный Lottie JSON (экспорт из Bodymovin).\n"
        ...
        self._create_accordion(self.tab_info, "⚙️", "ПОДГОТОВКА ИСХОДНОГО JSON", 
            "Правильный алгоритм работы в After Effects и плагин Bodymovie:\n\n"
            "1. Готовим анимацию к эспорту в PNG секвенцию\n"
            "Создайте новые композиции нужных размеров, например:\n"
            "270x270 px, 180x180 px, 90x90 px\n\n"
            ...
            "3. Импортируйте эти 3 папки обратно в After Effects\n (рекомендуется в новый чистый проект, чтобы ничего не смешалось).\n AE автоматически соберет из них композиции.\n\n"
            ...
            "6. Загрузите готовые JSON-файлы и папки images/ в эту программу.\n см. раздел «Как пользоваться приложением»")
        
        self._create_accordion(self.tab_info, "✨", "ОСОБЕННОСТИ СКРИПТА", 
            "• Автоматически исправляет параметр «op» последнего кадра\n  (частая ошибка Bodymovin)\n"
            "• Конвертирует изображения параллельно (быстро)\n"
            "• Встраивает всё в один JSON (base64)\n"
            "• Может сжимать в WebP для уменьшения веса\n\n"
            "• Добавлен новый алгоритм сжатия в современном формате AVIF\n"
            "• Скрипт умеет проверять исходный JSON файл на наличие некорректной подготовки перед экспортом из BodyMovie"
            "В логе есть [АВТОКОРРЕКЦИЯ] — это нормально.")
    ...
    def _check_for_updates(self):
        ...
        github_repo = "Griboed256/Lottie-Embed-Images"
        ...
        def check():
            ...
            try:
                ...
                raw_notes = data.get("body", "")

                # --- НОВАЯ ЛОГИКА ПАРСИНГА ОПИСАНИЯ ---
                short_notes_list = []
                for line in raw_notes.splitlines():
                    stripped_line = line.strip()
                    # Ищем строки, которые начинаются с 1, 2 или 3 решеток и пробела
                    if stripped_line.startswith("# ") or stripped_line.startswith("## ") or stripped_line.startswith("### "):
                        # Убираем все '#' слева и лишние пробелы
                        clean_title = stripped_line.lstrip("#").strip()
                        if clean_title:
                            # Добавляем красивый маркер списка для читаемости
                            short_notes_list.append(f"• {clean_title}")
                
                # Если в релизе не было ни одного заголовка, подставляем дефолтный текст
                if not short_notes_list:
                    release_notes = "Мелкие улучшения и исправления багов."
                else:
                    release_notes = "\n".join(short_notes_list)
                # ----------------------------------------
                ...
                # Если нет интернета или GitHub недоступен — просто молчим
                self._log_msg("  ⚠️ Проверка обновлений недоступна.")
