...
class LottieProcessor:
    ...
    def embed_images(
            ...
            use_webp: bool = False, quality: int = 85, lossless: bool = False,
            size_limit_mb: float = 0.0,
            log_fn: Callable[[str], None] = print,
            progress_fn: Optional[Callable[[int, int, float, float], None]] = None,
            cancel_check: Optional[Callable[[], bool]] = None # <--- Принимаем сигнал отмены
        ) -> Dict[str, Any]:
            ...
            if use_webp and not lossless and size_limit_mb > 0:
                limit_bytes = int(size_limit_mb * 1024 * 1024)
                log_fn(f"Подбираю качество под лимит {size_limit_mb} MB...")
                image_files = [item[2] for item in work_items]
                actual_quality, warn = ImageUtils.estimate_quality_for_limit(image_files, limit_bytes, lossless)
                if warn: log_fn(f"  ⚠️  ВНИМАНИЕ: качество {actual_quality} ниже {QUALITY_WARNING_THRESHOLD}.")

            log_fn(f"Режим: {'WebP lossless' if (use_webp and lossless) else f'WebP quality={actual_quality}' if use_webp else 'PNG'}  |  Изображений: {total}\n")

            def process_one(item: Tuple[int, dict, Path]) -> Tuple[int, str, int, int, str]:
                if cancel_check and cancel_check(): # Если отменили, возвращаем пустышку
                    return item[0], "", 0, 0, item[2].name
                    
                idx, asset, img_path = item
                raw = img_path.read_bytes()
                if use_webp:
                    converted = ImageUtils.png_bytes_to_webp(raw, quality=actual_quality, lossless=lossless)
                    mime = "image/webp"
                    final_bytes = converted
                else:
            ...
            with ThreadPoolExecutor(max_workers=workers) as pool:
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        continue # Пропускаем сломанный кадр, но не роняем программу
...
class PreviewServerManager:
    ...
    def start(self, output_jsons: List[Path], log_fn: Optional[Callable] = None) -> None:
        ...
        html_parts = [
            ...
            "<style>",
            "  body{margin:0;background:#1e1e2e;font-family:sans-serif;color:#cdd6f4;display:flex;flex-wrap:wrap;gap:30px;padding:40px;justify-content:center;align-items:flex-start;min-height:100vh;}",
            "  .card{background:#2a2a3e;padding:24px;border-radius:12px;display:flex;flex-direction:column;align-items:center;box-shadow:0 10px 15px -3px rgba(0,0,0,0.3);}",
            "  p{font-size:13px;opacity:.6;margin-top:16px;text-align:center;line-height:1.4;}",
            "  strong{color:#a6e3a1;font-weight:normal;font-size:14px}",
            "</style></head><body>"
        ]
        ...
        for i, p in enumerate(output_jsons):
            ...
            html_parts.append(f'''
            <div class="card">
                <div id="anim_{i}" style="width:{w}px;height:{h}px"></div>
                <p><strong>{p.name}</strong><br>{w} × {h} px</p>
                <script>
                  lottie.loadAnimation({{
                    container: document.getElementById('anim_{i}'),
                    renderer: 'canvas', loop: true, autoplay: true, path: '{req_path}'
                  }});
                </script>
            </div>
            ''')
        ...
        chosen_port, srv = None, None
        for port in (8000, 8080, 8888, 8001, 8081):
            try:
        ...
        if srv is None or chosen_port is None:
            log("  ❌ Не удалось запустить предпросмотр: порты заняты.")
            return
        ...
        if not self._wait_for_port(chosen_port):
            log("  ⚠️ Сервер запущен, но отвечает с задержкой.")

        url = f"http://localhost:{chosen_port}/test.html"
        log(f"  🌐 Галерея предпросмотра: {url}")
        webbrowser.open(url)
...
class LottieEmbedApp(TkBase):
    ...
    def _build_ui(self):
        ...
        ttk.Label(hdr, text="   JSON + PNG-секвенция → один self-contained файл", foreground=self.SUBTEXT, background=self.BG, font=("Segoe UI", 10)).pack(side="left", pady=(4, 0))
    ...
    def _get_quality_color(self, val):
        # Если JSON не выбран, принудительно отдаем серый цвет (#3A3A5C)
        if not getattr(self, 'is_estimation_active', False):
            return self.BORDER
            
        if val >= 85: return "#93D22F"
        if val >= 65: return "#B1E859"
        if val >= 50: return "#EFC44E"
        if val >= 40: return "#F0913D"
        if val >= 30: return "#C22222"
        return "#860F0F"
    ...
    def _build_settings_cards(self, parent):
        ...
        ttk.Label(self.c3, text="Применяется ко всем файлам в очереди", style="Sub.TLabel").pack(anchor="w", pady=(0, 6))
        for val, txt in [("webp", "WebP (рекомендуется) — меньший размер"), ("lossless", "WebP Lossless — без потерь"), ("png", "PNG — без конвертации")]:
...
# ── Водяной знак (правый нижний угол) ─────────────────────────────────
    ...
    def _update_preview_tab(self):
        ...
        try:
            ...
            self.lbl_preview_info.config(text=f"Размер: {w} × {h} px  |  Длительность: {frames_count} кадров. \n \n p.s предпросмотр может не корректно обображать FPS анимации. \n Корректно правильная анимация отображается только в браузере в конце рендера. \n")
    ...
    def _update_estimate(self):
        ...
        self.var_auto_quality.set("")
        
        # --- ФУНКЦИЯ СБРОСА UI ПРИ ПУСТЫХ ДАННЫХ ---
        def _reset_ui():
            self.is_estimation_active = False
            self.var_est_size.set("~ 0.00 MB")
            self.var_est_hint.set("Вставьте исходный JSON файл для начала расчётов.")
            self.limit_slider_color = self.BORDER
            if hasattr(self, 'quality_slider'): self.quality_slider.force_redraw()
            if hasattr(self, 'limit_slider'): self.limit_slider.force_redraw()
        ...
        # --- ЕСЛИ ДАННЫЕ ЕСТЬ — ОЖИВЛЯЕМ UI! ---
        self.is_estimation_active = True
        self.var_est_hint.set("Приблизительно, до обработки. Зависит от содержимого кадров.")
        if hasattr(self, 'quality_slider'): self.quality_slider.force_redraw()
        
        # Дальше начинается стандартный фоновый расчет...
        ...
        def _task():
            try:
                ...
                if self.var_use_limit.get():
                    best_q, best_est = 1, None
                    limit_bytes = int(limit * 1024 * 1024)
                    
                    for q in range(95, 0, -5):
                        # СПАСЕНИЕ ОТ ЗАВИСАНИЯ: Если пользователь сдвинул ползунок дальше, убиваем этот расчёт
                        if gen != self._est_generation: return 
                        
                        total_final = sum(len(ImageUtils.png_bytes_to_webp(p.read_bytes(), quality=q)) for p in samples)
                        est = (total_final / len(samples)) * total_count * (4 / 3)
                        if est <= limit_bytes:
                            best_q, best_est = q, est
                            break
                    
                    if not best_est:
                        raw_q1 = sum(len(ImageUtils.png_bytes_to_webp(p.read_bytes(), quality=1)) for p in samples)
                        best_est = (raw_q1 / len(samples)) * total_count * (4 / 3)
                        best_q = 1
                    
                    if gen == self._est_generation:
                        e_val = best_est / (1<<20)
                        self.after(0, lambda: self.var_est_size.set(f"≈ {e_val:.2f} MB"))
                        self.after(0, lambda: self.var_auto_quality.set(f"Авто WebP quality ≈ {best_q}"))
                        # Перекрашиваем ползунок!
                        self.after(0, lambda q=best_q: self._update_limit_slider_color(q))
                    return

                # Если лимит ВЫКЛЮЧЕН (работает ползунок Качества)
                total_final = sum(len(p.read_bytes()) if fmt == "png" else len(ImageUtils.png_bytes_to_webp(p.read_bytes(), quality=quality, lossless=(fmt == "lossless"))) for p in samples)
                
                if gen == self._est_generation:
                    est_mb = (total_final / len(samples)) * total_count * (4 / 3) / (1<<20)
                    self.after(0, lambda: self.var_est_size.set(f"~ {est_mb:.2f} MB"))
                    self.after(0, lambda: self.var_auto_quality.set("")) # Очищаем текст
        ...
        def _worker_thread():
            try:
                ...
                for idx, task in enumerate(tasks):
                    ...
                    # Добавляем _idx=idx, чтобы "заморозить" значение для каждого потока
                    ...
                    res = LottieProcessor.embed_images(
                        ...
                        use_webp=use_webp, quality=quality, lossless=lossless, size_limit_mb=limit_mb,
                        log_fn=self._log_msg, progress_fn=_local_progress,
                        cancel_check=lambda: self._cancel_conversion.is_set() # Передаем проверку внутрь
                    )
                    
                    # Сохраняем путь только если файл был доделан и не было отмены
                    if res.get("path") and not self._cancel_conversion: 
                        results_paths.append(res["path"])
                    
                    if total_tasks > 1 and not self._cancel_conversion:
                        self._log_msg(f"✅ Готово: {task['out_path'].name}\n")

                # --- ГЛАВНАЯ ПРОВЕРКА ПЕРЕД ФИНАЛОМ ---
                if self._cancel_conversion:
                    return # ВЫХОДИМ, НИЧЕГО НЕ ОТКРЫВАЯ

                # Открываем предпросмотр, только если все прошло штатно
                if self.var_preview.get() and results_paths:
                    self.server_manager.start(results_paths, log_fn=self._log_msg)
                
                # Открываем папки, только если не было отмены
